...
function StarRating({starRate, voteCount}) {
    ...
    return (
        <HFlex height='16px' etc='font-size : 16px'>
            ...
            arr.map((element) => {
                if(element){
                    return <Star src="img/star.png" key={uuid()}/>;
                } else {
                    return <Star src="img/halfstar.png" key={uuid()}/>;
                }
            })
            }
            <StRate>{starRate}</StRate>
            <span>{`(${voteCount})`}</span>
...
const Star = styled.img`
    height: 100%;
    filter : invert(65%) sepia(60%) saturate(563%) hue-rotate(3deg) brightness(91%) contrast(94%);
`;
